...
GridLayout {
	...
	content = [
		ColumnLayout {
			...
			content = [
				TextView { id = header1, text = "Alice’s Adventures in Wonderland", style = header1 },
				TextView { id = author, text = "by Lewis Carroll", semantics = blockquote, margin-bottom = 0.5em, text-size = 0.8em, text-align = center },
				TextView { id = header2, text = "CHAPTER I. Down the Rabbit-Hole", style = header2 },
			...
			content = [
				GridLayout {
					...
					content = [
						...
						Checkbox { row = 5, column = 0:1, id = headerSpanAll, content = "Header column-span-all" },
					]
				},
			]
		}
...
func createColumnLayoutDemo(session rui.Session) rui.View {
	...
	rui.Set(view, "headerSpanAll", rui.CheckboxChangedEvent, func(_ rui.Checkbox, checked bool) {
		rui.Set(view, "header1", rui.ColumnSpanAll, checked)
		rui.Set(view, "author", rui.ColumnSpanAll, checked)
		rui.Set(view, "header2", rui.ColumnSpanAll, checked)
	})
	return view
}
